      ...
      <div className="w-full grid grid-cols-2">
        <div className="w-[400px]">
          <div className="my-5 w-full">
            <ul className="category-list flex flex-col items-start gap-y-2">
              {productSettings.categories.map(({ id, name, image }) => (
                <li
                  key={id}
                  className="group cursor-pointer flex items-center gap-x-2"
                >
                  <div className="w-12 h-12 overflow-hidden rounded flex items-center font-semibold shadow justify-center relative">
                    {id !== undefined && (
                      <span
                        onClick={() => handleRemoveCategory(id)}
                        className="hidden group-hover:flex absolute top-0 left-0 w-full h-full rounded z-10 bg-white bg-opacity-50 backdrop-blur-sm items-center justify-center"
                      >
                        <SVGS.DeleteIcon color="red" />
                      </span>
                    )}
                    {image ? (
